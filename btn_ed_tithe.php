...
if (isset($_POST['id']) && isset($_POST['member_name']) && isset($_POST['tithe_amount']) && 
    isset($_POST['tithe_day']) && isset($_POST['member_phone']) && isset($_POST['member_address'])) {
    
    // Sanitize the input to prevent SQL injection
    $id = intval($_POST['id']);
    $name = $conn->real_escape_string($_POST['member_name']);
    $amount = $conn->real_escape_string($_POST['tithe_amount']);
    $day = $conn->real_escape_string($_POST['tithe_day']);
    $phone = $conn->real_escape_string($_POST['member_phone']);
    $address = $conn->real_escape_string($_POST['member_address']);
    
    // Prepare the UPDATE statement
    $sql = "UPDATE add_tithe SET name=?, amount=?, day=?, phone=?, address=? WHERE id=?";
